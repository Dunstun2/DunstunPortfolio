...
class AuthService {
  async login(email, password) {
    ...
    if (email === FALLBACK_ADMIN.email && (password === FALLBACK_ADMIN.password || password === 'ChangeMe123!')) {
      console.log('⚠️  Login using fallback admin account');
      const token = jwt.sign(
        { id: FALLBACK_ADMIN.id, email: FALLBACK_ADMIN.email, role: FALLBACK_ADMIN.role, isDefaultAdmin: true },
        JWT_SECRET,
        { expiresIn: '24h' }
      );
    ...
    const isDefaultAdmin = user.email === 'admin@example.com' || user.id === 0;

    const token = jwt.sign(
      { id: user.id, email: user.email, role: user.role, isDefaultAdmin },
      JWT_SECRET,
      { expiresIn: '24h' }
    );
