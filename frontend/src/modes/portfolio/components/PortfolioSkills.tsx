...
export default function SkillsSection({ variant = 'full' }: { variant?: 'highlights' | 'full' }) {
  ...
  const sectionTitle = settings?.skills_section_title || 'Skills & Expertise';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {isHighlights ? (
          <h2 className="text-3xl md:text-5xl font-bold text-heading-light mb-10 text-center">
            <ColoredTitle settingKey="skills_section_title" title={sectionTitle} />
          </h2>
        ) : (
          <div className="text-center mb-16">
            <h1 className="text-4xl md:text-6xl font-extrabold text-heading-light mb-4">
              <InlineText settingKey="skills_page_title" defaultValue="Skills & Expertise">
                {pageTitle}
              </InlineText>
            </h1>
            <p className="text-text-light text-lg max-w-2xl mx-auto">
              <InlineText settingKey="skills_page_subtitle" defaultValue="Technologies, tools, and capabilities I work with">
                {pageSubtitle}
              </InlineText>
            </p>
          </div>
          ...
          {displayCategories.map(([category, catSkills]) => {
            ...
            return (
              <div key={category} className="glass p-8 rounded-2xl">
                <h3 className="text-xl font-bold text-subheading mb-6 border-b border-text-light/15 pb-2">
