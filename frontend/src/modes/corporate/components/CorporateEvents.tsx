...
export default function EventsSection({ variant = 'full' }: { variant?: 'full' | 'highlights' }) {
  const { isInlineEditing } = useInlineEdit();
  const [events, setEvents] = useState<any[]>([]);
  const refreshKey = useRealtimeRefresh('events');

  useEffect(() => {
    const endpoint = variant === 'highlights' ? '/events/published?limit=4' : '/events/published';
    fetchApi(endpoint)
  ...
  return (
    <section id="events" className="py-4 md:py-8 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* View All Events Button (only for highlights) */}
        {variant === 'highlights' && (
